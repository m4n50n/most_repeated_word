...
str = str.toLowerCase(); // Convertimos todo a minúsculas para poder comparar correctamente palabras minúsculas y mayúsculas
str = str.replaceAll(",", ""); // Eliminamos comas
str = str.replaceAll(".", ""); // Eliminamos puntos

// En este texto en concreto no es necesario, pero de manera genérica podrían también eliminarse cualquier caracter especial o símbolo que no fuera una letra desde la a a la z

let str_array = str.split(" "); // Crear un array cuyos elementos se correspondan con cada palabra (separada por un espacio) que haya en el párrafo
let words = {} // Definir un objeto que contendrá propiedades cuyo nombre sea la palabra y cuyo valor sea la cantidad de veces que aparece en el texto

for (index in str_array) {
    let value = str_array[index];
    if (value.length == 0) { continue; } // En caso de que en el párrafo existieran varios espacios seguidos, a la hora de hacer el split creará elementos vacios, por lo tanto si en este bucle nos encontramos con un elemento vacio, no haremos nada y pasaremos al siguiente ciclo del bucle

    (words[value] == undefined) ? words[value] = 1 : words[value] += 1; // Si la palabra aún no existe en el objeto, la añadiremos con valor 1 (primera repetición), y si ya existe le sumaroms uno al valor
}
...
console.log("Solución 1: La palabra que más se repite es '" + max + "' con un total de " + words[max] + " veces.");
console.log("Solución 2: La palabra que más veces se repite es '" + result[0] + "' con un total de " + result[1] + " veces.");
